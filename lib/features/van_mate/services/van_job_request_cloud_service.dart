...
class VanJobRequestCloudService {
  ...
  Future<VanJobRequestRecord> createOrUpdateFromDraft({
    ...
    final request = VanJobRequestRecord(
      ...
      customerJourneyType: draft.customerJourneyType.trim().isEmpty
          ? 'quote'
          : draft.customerJourneyType.trim(),
      startHandover: draft.startHandover.trim(),
      endHandover: draft.endHandover.trim(),
      allowedStartHandoverOptions: draft.allowedStartHandoverOptions,
      allowedEndHandoverOptions: draft.allowedEndHandoverOptions,
      collectionAddress: draft.collectionAddress.trim(),
      returnAddress: draft.returnAddress.trim(),
      returnAddressSameAsCollection: draft.returnAddressSameAsCollection,
      businessDropOffInstructions: draft.businessDropOffInstructions.trim(),
  ...
  Future<VanJobRequestRecord?> submitCustomerReply({
    ...
    final updated = VanJobRequestRecord(
      ...
      customerJourneyType: existing.customerJourneyType,
      startHandover: existing.startHandover,
      endHandover: existing.endHandover,
      allowedStartHandoverOptions: existing.allowedStartHandoverOptions,
      allowedEndHandoverOptions: existing.allowedEndHandoverOptions,
      collectionAddress: existing.collectionAddress,
      returnAddress: existing.returnAddress,
      returnAddressSameAsCollection: existing.returnAddressSameAsCollection,
      businessDropOffInstructions: existing.businessDropOffInstructions,
      businessCollectionInstructions: existing.businessCollectionInstructions,
